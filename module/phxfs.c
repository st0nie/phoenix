...
static int phxfs_devm_memremap(struct phxfs_dev* phx_dev)
{
    ...
    phx_dev->p2p_pgmap =
        devm_kzalloc(&phx_dev->dev->dev, sizeof(struct pci_p2pdma_pagemap), GFP_KERNEL);
    if (phx_dev->p2p_pgmap == NULL) return -ENOMEM;
...
static int phxfs_ctrl_init(struct phxfs_ctrl* dev_ctrl, u32 dev_num)
{
    ...
    for (i = 0; i < dev_ctrl->dev_num; i++) {
        ...
        for (j = 0; j < PCI_STD_NUM_BARS; j++) {
            ...
            if (size > 0) {
                printk(KERN_INFO "phxfs: Resource %d: start %llx size %llx\n", j,
                       (u64)pci_resource_start(dev_ctrl->phx_dev[i].dev, j), size);
            }
...
static long phxfs_ioctl(struct file* filp, unsigned int cmd, unsigned long arg)
{
    void __user* argp = (void*)arg;
    switch (cmd) {
        case PHXFS_IOCTL_MAP: {
            struct phxfs_ioctl_map_s map_param;
            if (copy_from_user(&map_param, argp, sizeof(struct phxfs_ioctl_map_s))) return -EFAULT;
...
void phxfs_cdev_del(struct cdev* cdev, struct device* cdev_device, struct phxfs_dev* dev)
{
    ...
    if (dev->p2p_pgmap != NULL) { devm_kfree(&dev->dev->dev, &dev->p2p_pgmap->pgmap); }
    dev->dev = NULL;
    ida_simple_remove(&phxfs_chr_minor_ida, dev->idx);
}

int phxfs_cdev_add(struct cdev* cdev, struct device* cdev_device,
                   const struct file_operations* fops, struct module* owner, struct phxfs_dev* dev)
{
    int ret;
    ...
    if (ret) ida_simple_remove(&phxfs_chr_minor_ida, dev->idx);
    return ret;
}
...
int phxfs_cdev_init(struct phxfs_ctrl* ctrl)
{
    int ret = -ENOMEM;
    int i;
    ret = alloc_chrdev_region(&phxfs_chr_devt, 0, ctrl->dev_num, "phxfs-generic");
    if (ret < 0) goto destroy_subsys_class;
    ...
    for (i = 0; i < ctrl->dev_num; i++) {
        ret = phxfs_cdev_add(&ctrl->phx_dev[i].cdev, &ctrl->phx_dev[i].device, &phxfs_chr_fops,
                             THIS_MODULE, &ctrl->phx_dev[i]);
        if (ret) {
